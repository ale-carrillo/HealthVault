...
export default function PatientDialog({
    ...
    const savePatient = async () => {
        ...
        if (action === "add") {
            const response = await axios.post("http://localhost:8004/api/v1/patient",patient)
            setRows((prevRows) => [...prevRows, response.data]);
            setAlert({ message: "Patient added successfully", severity: "success" });
        } else if (action === "edit") {
            const response = await axios.put(`http://localhost:8004/api/v1/patient/${patient._id}`,patient)
